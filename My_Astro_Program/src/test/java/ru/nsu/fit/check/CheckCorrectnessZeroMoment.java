...
class CheckCorrectnessZeroMoment {
    private static final int RADIUS = 8;

    private static final Logger LOGGER = LogManager.getLogger(CheckCorrectnessZeroMoment.class);
    ...
    public void runSlidingWindow(double[][] normalizedMatrix, TiffProcessor tiffProcessor) {
        ...
        for (int x = RADIUS; x < rows - RADIUS; x++) {
            for (int y = RADIUS; y < cols - RADIUS; y++) {
                ...
                tiffProcessor.highlightPixel(y, x, normalizeModule(sum, 175));
                if (maxDiff < sum) {
                    maxDiff = sum;
                }
                if (minDiff > sum) {
